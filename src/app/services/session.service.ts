...
export class SessionService {
  ...
  get parallelSessions(): SessionResource[] | undefined {
    if (this._parallelSessions.length > 0) return this._parallelSessions;
    else {
      return undefined;
    }
  }
  ...
  public getSessionsByDay(day: string): SessionResource[] | undefined {
    return (
      this._sessions?.filter((p) =>
        moment(p.startDateTimeUtc).isSame(moment(day), 'day')
      ) ?? []
    );
  }

  public getSessionsByDayandFilteredWithParallelSessions(
    day: string
  ): SessionResource[] | undefined {
    let parallelSessions = this.getParallelSessions(
    ...
    if (parallelSessions?.length == 0) {
      ...
      return this._sessions
        ?.filter((p) => moment(p.startDateTimeUtc).isSame(moment(day), 'day'))
        .filter((item) => !parallelSessions?.includes(item));
    }
  }

  public getSessionsBefore(session: SessionResource): SessionResource[] {
    return this.getSessionsByDay(
      session.startDateTimeUtc.split('T')[0]
    )!.filter(
      (p) =>
  ...
  public getOffsetToPreviousSessionInHours(session: SessionResource): number {
    let parallelSessions: SessionResource[] = this.getParallelSessions(
      this.getSessionsByDay(session.startDateTimeUtc.split('T')[0])!
    );
    let previousSession: SessionResource[] = [];
    if (parallelSessions.length == 0) {
      previousSession = this.getSessionsByDay(
        session.startDateTimeUtc.split('T')[0]
      )!
        .sort((p) => moment(session.endDateTimeUtc).diff(p.startDateTimeUtc))
        .filter((p) =>
          moment(p.startDateTimeUtc).isBefore(session.endDateTimeUtc)
        )
    ...
    if (parallelSessions.includes(session)) {
      ...
      previousSession = this.getSessionsByDayandFilteredWithParallelSessions(
        session.startDateTimeUtc.split('T')[0]
      )!
        .sort((p) => moment(session.endDateTimeUtc).diff(p.startDateTimeUtc))
        .filter((p) =>
    ...
    if (previousSession.length > 0) {
      const duration = moment.duration(
        moment(session.startDateTimeUtc).diff(
          moment(previousSession[previousSession.length - 1].endDateTimeUtc)
        )
      );
  ...
  public getParallelSessions(sessions: SessionResource[]): SessionResource[] {
    ...
    const sortedSessions = sessions.sort((a) =>
      moment(a.startDateTimeUtc).valueOf()
    );

    for (let i = 0; i < sortedSessions.length; i++) {
      let checkSession = sortedSessions[i];
      const otherSessions = sortedSessions.filter(
        (p) => p.id != checkSession.id
      );
    ...
    for (let i = 0; i < pSessions.length; i++) {
      ...
      if (!p2Sessions.includes(firstSession)) {
        p2Sessions.push(
          ...this.findOverlapSession(otherSessions, firstSession)
        );
      }
    }
  ...
  public getStartOffset(session: SessionResource): number {
    const sortedSessions = this.getSessionsByDay(
      session.startDateTimeUtc.split('T')[0]
    ...
    const firstSessionPerTrack = this._distinctByTrack(sortedSessions).sort(
      (a) => moment(a.startDateTimeUtc).valueOf()
    );

    if (parallelSessions.includes(session) && parallelSessions[0] == session) {
      const duration = moment.duration(
        moment(session.startDateTimeUtc).diff(
          moment(sortedSessions[0].startDateTimeUtc)
        )
      );
      return duration.asHours();
    }
    ...
    if (firstSessionPerTrack.length > 0) {
      const duration = moment.duration(
        moment(session.startDateTimeUtc).diff(
          moment(firstSessionPerTrack[0].startDateTimeUtc)
        )
      );
  ...
  public async getSessions(): Promise<void> {
    const result = await firstValueFrom(this.getSessionFromApi(this._eventId));
    this._sessions = result.result.filter((p) => p.isMainSession == false);
    this._days = [
      ...new Set(
        this._sessions?.map(
          (s) => moment(s.startDateTimeUtc).toISOString().split('T')[0]
        )
      ),
    ].sort();
  }
  ...
  public getSessionFromApi(
    eventId: string
  ): Observable<PagedResult<SessionResource>> {
    return this._http.get<PagedResult<SessionResource>>(
      `${this._settings.settings.backendUrl}/tenant/${this._tenantId}/Publishing/Event/${eventId}/sessions`,
      {
  ...
  private isAnOverlapSession(
    sessionArray: SessionResource[],
    ...
    for (let i = 0; i < sessionArray.length; i++) {
      ...
      if (
        session.startDateTimeUtc > eventA.startDateTimeUtc &&
        session.startDateTimeUtc < eventA.endDateTimeUtc
      ) {
        return true;
      }
      //end-time in between any of the events
      if (
        session.endDateTimeUtc > eventA.startDateTimeUtc &&
        session.endDateTimeUtc < eventA.endDateTimeUtc
      ) {
        return true;
      }
